...
t = open("transposon_relationship_input.csv", 'w')
# Header
t.writelines('speciesid' + ',' + 'class' + ',' + 'order' + ',' + 'superfamily' + ',' + 'header' + ',' + 'sequence' + ',' + 'length' + ',' + 'count' + ',' + 'coverage' + '\n')
...
def parse_record(records, counts_filepath, species_id):
	for record in records:
		...
		# collect information from counts_file
		countsfile = open(counts_filepath, 'r')
		# ignore header
		countsfile.readline()
		count = 'Unknown'
		coverage = 'Unknown'
		for line in countsfile:
			if line.split('\t')[0] in orig_header:
				count = line.split('\t')[1]
				coverage = line.split('\t')[2]
				break

		t.writelines(str(species_id) + ',' + myclass + ',' + order + ',' + superfamily + ',' + header + ',' + sequence + ',' + length + ',' + str(count) + ',' + str(coverage) + '\n')
	print("done parsing")


parse_record(records = SeqIO.parse("Repeats_AeAlbo.fa.txt", "fasta"), counts_filepath = 'AeAlbo_TE_count.txt', species_id = 4)
parse_record(records = SeqIO.parse("Repeats_AeAeg.fa.txt", "fasta"), counts_filepath = 'AeAeg_TE_count.txt', species_id = 1)
parse_record(records = SeqIO.parse("Repeats_CuQuin.fa.txt", "fasta"), counts_filepath = 'CuQuin_TE_count.txt', species_id = 3)
parse_record(records = SeqIO.parse("Repeats_AnGam.fa.txt", "fasta"), counts_filepath = 'AnGam_TE_count.txt', species_id = 2)
